...
*让你的 AI 智能体拥有真正的记忆力——跨会话、跨智能体、跨时间。当前独立项目版额外提供 Dreaming 兼容输出。*
...
| **Dreaming 兼容** | 输出 `DREAMS.md` 与 `.dreams/*`，让 `memory-lancedb-pro` 与 Dreaming UI 同时可用 |
...
- `dreaming.enabled + frequency` → 为 Dreaming 页生成 `DREAMS.md` 与短期记忆产物
...
如果你现在使用的是这份增强后的独立项目版，建议优先看下面几份文档：

- [独立项目说明](docs/standalone-project.zh-CN.md)
- [Dreaming 兼容设计说明](docs/dreaming-compatibility.zh-CN.md)
- [记忆架构分析](docs/memory_architecture_analysis.md)
- [OpenClaw 集成与迭代手册](docs/openclaw-integration-playbook.zh-CN.md)

推荐阅读顺序：

1. 先看独立项目说明，理解这份项目的边界和目录结构
2. 再看 Dreaming 文档，理解 `memory-lancedb-pro` 如何兼容 Dreaming UI
3. 再看架构分析，理解主链路
4. 最后看集成手册，处理真实接入和排障
